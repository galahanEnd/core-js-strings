...
function removeFirstOccurrences(str, value) {
  const strMatch = str.indexOf(value);
  if (strMatch === -1) return str;
  return str.slice(0, strMatch) + str.slice(strMatch + value.length);
}
...
function removeLastOccurrences(str, value) {
  const strMatch = str.lastIndexOf(value);
  if (strMatch === -1) return str;
  return str.slice(0, strMatch) + str.slice(strMatch + value.length);
}
...
function countVowels(str) {
  ...
  return str.split('').reduce((count, char) => {
    return count + (vowels.includes(char.toLowerCase()) ? 1 : 0);
  }, 0);
}
...
function isPalindrome(str) {
  const norml = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const rev = norml.split('').reverse().join('');
  return norml === rev;
}
...
function findLongestWord(sentence) {
  ...
  return words.reduce((longest, word) => {
    if (longest.length < word.length) return word;
    return longest;
  }, '');
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((letter) => {
      if (alfUpper.includes(letter)) {
        const pos = alfUpper.indexOf(letter);
        const newPos = (pos + 13) % 26;
        return alfUpper[newPos];
      }
      if (alfLower.includes(letter)) {
        const pos = alfLower.indexOf(letter);
        const newPos = (pos + 13) % 26;
        return alfLower[newPos];
      }
      return letter;
    })
    .join('');
